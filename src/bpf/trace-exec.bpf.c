...
static int bpf_copy_strv (struct bpf_strv *strv, unsigned long cnt, long start, long end)
{
	strv->cnt  = cnt;
	strv->size = (end - start) & (BPF_STRV_SIZE - 1);;

	return bpf_probe_read_user(strv->buf, strv->size, (void *)start);
}
...
int BPF_PROG(trace_process_exec, struct task_struct *p, pid_t old_pid, struct linux_binprm *bprm)
{
	...
	ret = bpf_copy_strv(&ev->args, bprm->argc, mm->arg_start, mm->arg_end);
	if (ret < 0)
		goto error;

	ret = bpf_copy_strv(&ev->env, bprm->envc, mm->env_start, mm->env_end);
	if (ret < 0)
		goto error;
